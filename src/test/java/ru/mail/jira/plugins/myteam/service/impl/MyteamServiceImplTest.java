...
class MyteamServiceImplTest {
  ...
  void sendMessageWhenMessageIsEmptyString(String message) {
    ...
    // WHEN // THEN
    assertThrows(
        IllegalArgumentException.class, () -> myteamService.sendMessage(applicationUser, message));
  }
  ...
  void createChatByJiraApplicationUsersWhenChatNameEmptyString(String chatName) {
    ...
    assertThrows(
        IllegalArgumentException.class,
        () ->
            myteamService.createChatByJiraApplicationUsers(
                issueKeyLinkToChat, chatName, jiraUsers, loggedInUser, isPublic));
  }
  ...
  @Test
  void
      createChatByJiraApplicationUsersWhenListUserNotEmptyAndChatWasCreatedWithoutSettingUpdatedAboutChat()
          throws LinkIssueWithChatException, MyteamServerErrorException, IOException {
    // GIVEN
    String issueKeyLinkToChat = "ABC-123";
    String chatName = "someChatName";
    ApplicationUser user = mock(ApplicationUser.class);
    ...
    when(i18nResolver.getText(
            eq("ru.mail.jira.plugins.myteam.createChat.about.text"),
            eq(""),
            eq("baseurl/browse/ABC-123")))
        .thenReturn("someAbout with link issue baseurl/browse/ABC-123");
    ...
    when(httpResponse.getStatus()).thenReturn(200);
    when(myteamApiClient.createChat(
            eq("someToken"),
            eq(chatName),
            eq("someAbout with link issue baseurl/browse/ABC-123"),
            anyList(),
            eq(isPublic)))
        .thenReturn(httpResponse);
    String someFirstMessageInChatAfterCreating = "some first message";
    when(i18nResolver.getRawText(
            eq("ru.mail.jira.plugins.myteam.myteamEventsListener.groupChat.all.commands")))
        .thenReturn(someFirstMessageInChatAfterCreating);
    when(myteamApiClient.getChatInfo(eq("someChatId")))
        .thenThrow(new MyteamServerErrorException(500, "error!"));
    ...
    verify(myteamApiClient)
        .sendMessageText(eq("someChatId"), eq(someFirstMessageInChatAfterCreating));
  ...
  @EmptySource
  void
      createChatByJiraApplicationUsersWhenListUserEmptyAndChatWasCreatedWithoutSettingUpdatedAboutChat(
  ...
  @Test
  void createChatByJiraUserIdsWhenFirstMessageWillNotSent()
      throws MyteamServerErrorException, IOException, LinkIssueWithChatException {
    // GIVEN
    String issueKeyLinkToChat = "ABC-123";
    String chatName = "someChatName";
    ...
    when(myteamApiClient.sendMessageText(
            eq(createdChatId), eq(someFirstMessageInChatAfterCreating)))
        .thenThrow(new MyteamServerErrorException(500, "error!"));
    ...
    when(userSearchService.findUsersByEmail(eq("someEmail")))
        .thenReturn(Collections.singletonList(user));
    when(avatarService.getAvatarURL(eq(loggedInUser), eq(user)))
        .thenReturn(URI.create("http://localhost:8080/some_avatar_url"));
    when(i18nResolver.getText(
            eq("ru.mail.jira.plugins.myteam.createChat.about.text"),
            eq(chatInfoResponse.getInviteLink()),
            eq("baseurl/browse/ABC-123")))
        .thenReturn("updated someAbout with link issue baseurl/browse/ABC-123");

    // WHEN
    myteamService.createChatByJiraUserIds(
        issueKeyLinkToChat, chatName, jiraUsers, loggedInUser, isPublic);
    ...
    verify(myteamApiClient)
        .setAboutChat(
            eq(someBotToken),
            eq(createdChatId),
            eq("updated someAbout with link issue baseurl/browse/ABC-123"));
  }

  @ParameterizedTest
  @MethodSource(value = "getBadResponseForCreatingChat")
  void createChatByJiraUserIdsWhenChatNotCreated(
      HttpResponse<CreateChatResponse> createChatResponseHttpResponse)
      throws MyteamServerErrorException, IOException, LinkIssueWithChatException {
    // GIVEN
    String issueKeyLinkToChat = "ABC-123";
    String chatName = "someChatName";
    ...
    when(myteamApiClient.createChat(
            ...
            anyList(),
            eq(isPublic)))
        .thenReturn(createChatResponseHttpResponse);
  ...
  void createChatByJiraUserIdsWhenChatWasCreatedAndGettingAboutChatReturnBadHttpResponse(
      HttpResponse<ChatInfoResponse> chatInfoResponseHttpResponse)
      throws MyteamServerErrorException, IOException, LinkIssueWithChatException {
    // GIVEN
    String issueKeyLinkToChat = "ABC-123";
    String chatName = "someChatName";
  ...
  @Test
  void createChatByJiraUserIdsWhenWholeFlowSuccess()
      throws MyteamServerErrorException, IOException, LinkIssueWithChatException {
    // GIVEN
    String issueKeyLinkToChat = "ABC-123";
    String chatName = "someChatName";
  ...
  private static Stream<Arguments> getBadResponseForCreatingChat() {
    ...
    when(badResponse3.getParsingError())
        .thenReturn(
            Optional.of(
                new UnirestParsingException(badOriginalBody, someInternalParsingException)));
    return Stream.of(
        Arguments.of(badResponse1), Arguments.of(badResponse2), Arguments.of(badResponse3));
  }
  ...
  public static Stream<Arguments> getBadResponseForGettingAboutChatInfo() {
    ...
    RuntimeException someInternalParsingException = new RuntimeException();
    when(badResponse2.getBody()).thenReturn(null);
    when(badResponse2.getParsingError())
        .thenReturn(
            Optional.of(
                new UnirestParsingException(badOriginalBody, someInternalParsingException)));
