        ...
        try {
            ...
            if (issueToShow != null) {
                if (permissionManager.hasPermission(
                    ...
                    HttpResponse<MessageResponse> response = myteamApiClient.sendMessageText(
                            chatId,
                            messageFormatter.createIssueSummary(issueToShow, user),
                            messageFormatter.getIssueButtons(issueToShow.getKey(), user));
                    if (response.getStatus() != 200 || response.getBody() != null && !response.getBody().isOk()) {
                        log.warn("sendIssueViewToUser({}, {}, {}). Text={} Response={}", issueKey, user, chatId, messageFormatter.createIssueSummary(issueToShow, user), response.getBody().toString());
                    }
                } else {
                    myteamApiClient.sendMessageText(
                            chatId,
