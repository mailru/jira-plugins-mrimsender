...
public class BotFaultToleranceProvider implements LifecycleAware {
  ...
  private static final JobId CHOOSE_MAIN_NODE_JOB_ID =
      JobId.of(BotFaultToleranceProvider.class.getName() + "- choose main node");

  private static final JobRunnerKey CHECK_BOT_WORKS_JOB =
      JobRunnerKey.of(BotFaultToleranceProvider.class + "- global cluster job");
  private static final JobId CHECK_BOT_WORKS_JOB_ID =
      JobId.of(BotFaultToleranceProvider.class.getName() + "- check bot works");
  ...
  @Autowired
  public BotFaultToleranceProvider(
  ...
  private JobConfig registerChooseMainNodeJob() {
              ...
              if (StringUtils.isNotBlank(pluginData.getMainNodeId())) {
                ...
                for (Node node : clusterManager.findLiveNodes()) {
                  if (node.getNodeId() != null
                      && node.getNodeId().equals(pluginData.getMainNodeId())) {
                ...
                log.debug(
                    "ClusterManager node id = {}, Main node id from plugin data = {}",
                    clusterManager.getNodeId(),
                    pluginData.getMainNodeId());
              }
            } finally {
              clusterLock.unlock();
            }
    ...
    return JobConfig.forJobRunnerKey(CHOOSE_MAIN_NODE_JOB)
        .withSchedule(
            Schedule.forCronExpression("0 0/1 * ? * *", timeZoneManager.getDefaultTimezone()))
  ...
  private JobConfig registerCheckBotWorksJob() {
        ...
        new JobRunner() {
          ...
          public JobRunnerResponse runJob(JobRunnerRequest jobRunnerRequest) {
            if (!clusterManager.isClustered()
                && !myteamBot.isFetcherRunning()
                && pluginData.getToken() != null) {
              // this is server instance, restart bot if it was turned off
              log.debug(
                  "Found out that current bot session isn't running, so the session was restarted");
              myteamBot.startRespondingBot();
            }
            if (clusterManager.isClustered()) {
              // data center
              ...
              try {
                ...
                if (pluginData.getMainNodeId().equals(clusterManager.getNodeId())) {
                  ...
                  // checks our bot service is alive and didn't break because of telegram RLE for
                  // example
                  if (!myteamBot.isFetcherRunning()) {
                    log.debug(
                        "Found out that current bot session isn't running, so restart the session");
                    myteamBot.startRespondingBot();
                  }
                }
              } finally {
    ...
    return JobConfig.forJobRunnerKey(CHECK_BOT_WORKS_JOB)
        .withSchedule(
            Schedule.forCronExpression("30 0/1 * ? * *", timeZoneManager.getDefaultTimezone()))
  ...
  public void onStop() {
    if (clusterManager.isClustered()) {
      if (clusterManager.getNodeId() != null
          && clusterManager.getNodeId().equals(pluginData.getMainNodeId()))
