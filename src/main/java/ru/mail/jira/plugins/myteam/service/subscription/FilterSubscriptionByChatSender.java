...
@Component
public class FilterSubscriptionByChatSender extends AbstractFilterSubscriptionSender {

  @Autowired
  public FilterSubscriptionByChatSender(
  ...
  public void sendMyteamNotifications(final FilterSubscription subscription) {
    for (final String chat : CommonUtils.split(subscription.getRecipients())) {
      this.tryGetChatInfo(subscription, chat);
      super.sendMessages(subscription, null, chat);
    }
  }

  private void tryGetChatInfo(final FilterSubscription subscription, final String chat) {
    try {
      userChatService.getChatInfo(chat);
    } catch (Exception e) {
      super.sendMessage(
          i18nResolver.getText(
              "ru.mail.jira.plugins.myteam.subscriptions.page.subscription.error.chat.notExist",
              chat),
          userManager.getUserByKey(subscription.getUserKey()),
          null,
          null);
    }
  }
