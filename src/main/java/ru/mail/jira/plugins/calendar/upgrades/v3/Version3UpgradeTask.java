...
public class Version3UpgradeTask implements ActiveObjectsUpgradeTask {
    ...
    public void upgrade(ModelVersion currentVersion, final ActiveObjects ao) {
        ...
        if (currentVersion.isOlderThan(getModelVersion())) {
            ao.migrate(Permission.class, Calendar.class, Permission.class, UserCalendar.class, UserData.class);
                    ...
                    public Calendar doInTransaction() {
                        ...
                        for (LocalShare localShare : calendarShares) {
                            ...
                            if (localShare.groupName != null)
                                permission.setGroup(localShare.groupName);
                            else {
                                permission.setProject(localShare.projectId);
                                if (localShare.roleId != null)
                                    permission.setRole(localShare.roleId);
                            }
        ...
        for (ApplicationUser user : userManager.getAllApplicationUsers()) {
            ...
            try {
                ...
                for (final long oldCalendarId : oldToNewCalendar.keySet()) {
                    ...
                    if (calendar.getAuthorKey().equals(userKey)) {
                    ...
                    } else if (calendar.getPermissions() != null && calendar.getPermissions().length > 0) {
                        ...
                        for (Permission permission : permissions) {
                            if (permission.getGroup() != null) {
                                ...
                                Project project = projectManager.getProjectObj(permission.getProject());
                                if (project != null) {
                                    if (permission.getRole() != 0) {
                                        ProjectRole projectRole = projectRoleManager.getProjectRole(permission.getRole());
