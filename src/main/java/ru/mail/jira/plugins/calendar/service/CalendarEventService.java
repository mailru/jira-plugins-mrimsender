...
public class CalendarEventService {
    ...
    public List<Event> findEvents(final int calendarId,
        ...
        DateTimeFormatter dateFormatter = dateTimeFormatter.forUser(user).withStyle(DateTimeStyle.ISO_8601_DATE);
        ...
        if (source.startsWith("project_"))
            return getProjectEvents(calendarModel, Long.parseLong(source.substring("project_".length())),
                                    calendarModel.getEventStart(), calendarModel.getEventEnd(), dateFormatter.parse(start), dateFormatter.parse(end), user, includeIssueInfo);
